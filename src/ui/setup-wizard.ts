...
function updateWizardDisplay(): void {
  // Hide all steps
...
function validateCurrentStep(): boolean {
  switch (currentStep) {
    ...
    case 3: {
      ...
      const autoHighlight = document.getElementById(
        'auto-highlight'
      ) as HTMLInputElement;
...
function initializeWizard(): void {
  ...
  if (learningLanguageSelect && nativeLanguageSelect) {
    const updateNextButton = () => {
      ...
      if (nextButton) {
        const hasSelection =
          learningLanguageSelect.value && nativeLanguageSelect.value;
...
(window as any).updateDifficultyValue = updateDifficultyValue;
