...
import { CURRENT_SCHEMA_VERSION } from './storage-manager';
...
export class DataMigrator {
  ...
  async migrateToVersion(targetVersion: string): Promise<void> {
    ...
    if (compareVersions(currentVersion, targetVersion) > 0) {
      throw new Error(
        `Cannot downgrade from ${currentVersion} to ${targetVersion}`
      );
    }
    ...
    try {
      ...
      for (const migration of MIGRATIONS) {
        ...
        if (
          ...
          console.log(
            `Applying migration to ${migration.version}: ${migration.description}`
          );
  ...
  validateSchema(data: unknown): boolean {
    ...
    const schema = data as Partial<StorageSchema>;
    ...
    if (
      typeof settings.nativeLanguage !== 'string' ||
    ...
    if (
      typeof stats.articlesProcessed !== 'number' ||
  ...
  private async handleMigrationFailure(
    ...
    console.error(
      `Failed to migrate from version ${previousVersion}. Data may be in an inconsistent state.`
    );
  }
   ...
   */
  async testMigration(targetVersion: string): Promise<{
    success: boolean;
    errors: string[];
  }> {
    const errors: string[] = [];
