...
URL Source: https://developer.chrome.com/docs/extensions/reference/api/events
...
Declarative event handlers are used for example in the [Declarative Content API](https://developer.chrome.com/docs/extensions/reference/api/declarativeContent). This page describes the underlying concepts of all declarative event handlers.
...
Event objects may support rules. These event objects don't call a callback function when events happen but test whether any registered rule has at least one fulfilled condition and execute the actions associated with this rule. Event objects supporting the declarative API have three relevant methods: [`events.Event.addRules()`](https://developer.chrome.com/docs/extensions/reference/api/events#method-Event-addRules), [`events.Event.removeRules()`](https://developer.chrome.com/docs/extensions/reference/api/events#method-Event-removeRules), and [`events.Event.getRules()`](https://developer.chrome.com/docs/extensions/reference/api/events#method-Event-getRules).
...
If the rules were inserted successfully, the `details` parameter contains an array of inserted rules appearing in the same order as in the passed `rule_list` where the optional parameters `id` and `priority` were filled with the generated values. If any rule is invalid, for example, because it contained an invalid condition or action, none of the rules are added and the [runtime.lastError](https://developer.chrome.com/docs/extensions/reference/api/runtime#property-lastError) variable is set when the callback function is called. Each rule in `rule_list` must contain a unique identifier that is not already used by another rule or an empty identifier.
...
**Prefer substring matching over regular expressions in an [events.UrlFilter](https://developer.chrome.com/docs/extensions/reference/api/events#type-UrlFilter).** Substring based matching is extremely fast.
...
When matching URLs (as in the example above), event filters support the same URL matching capabilities as expressible with a [`events.UrlFilter`](https://developer.chrome.com/docs/extensions/reference/api/events#type-UrlFilter), except for scheme and port matching.
...
(rules: [Rule](https://developer.chrome.com/docs/extensions/reference/api/events#type-Rule)<anyany>[], callback?: function)=>{...}
...
(rules: [Rule](https://developer.chrome.com/docs/extensions/reference/api/events#type-Rule)<anyany>[])=> void
