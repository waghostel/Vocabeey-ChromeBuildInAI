...
URL Source: https://developer.chrome.com/docs/extensions/reference/api/proxy
...
You must declare the "proxy" permission in the [extension manifest](https://developer.chrome.com/docs/extensions/mv3/manifest) to use the proxy settings API. For example:
...
A proxy server is configured in a [`proxy.ProxyServer`](https://developer.chrome.com/docs/extensions/reference/api/proxy#type-ProxyServer) object. The connection to the proxy server (defined by the `host` attribute) uses the protocol defined in the `scheme` attribute. If no `scheme` is specified, the proxy connection defaults to `http`.

If no `port` is defined in a [`proxy.ProxyServer`](https://developer.chrome.com/docs/extensions/reference/api/proxy#type-ProxyServer) object, the port is derived from the scheme. The default ports are:
...
The next snippet queries the current effective proxy settings. The effective proxy settings can be determined by another extension or by a policy. See the [Types API](https://developer.chrome.com/docs/extensions/reference/api/types#type-ChromeSetting) documentation for details.
