...
export class StorageManager {
     ...
     */
    async getArticleByUrl(url, language) {
        // First check cache
        ...
        const article = articles.find(a => a.url === url && a.originalLanguage === language);
        if (article) {
    ...
    async saveVocabulary(vocab) {
        ...
        await this.cacheManager.cacheVocabularyTranslation(vocab.word, settings.learningLanguage, settings.nativeLanguage, vocab.translation);
    ...
    async checkStorageQuota() {
        ...
        if (percentUsed >= STORAGE_QUOTA_CRITICAL_THRESHOLD) {
            status = 'critical';
            console.error(`Storage quota critical: ${(percentUsed * 100).toFixed(1)}% used`);
        }
        else if (percentUsed >= STORAGE_QUOTA_WARNING_THRESHOLD) {
            status = 'warning';
            console.warn(`Storage quota warning: ${(percentUsed * 100).toFixed(1)}% used`);
        }
        return {
            bytesInUse,
            quota,
    ...
    async importData(data) {
        try {
            ...
            if (parsedData.schema_version !== CURRENT_SCHEMA_VERSION) {
                console.warn(`Importing data from different schema version: ${parsedData.schema_version}`);
            }
    ...
    async performMaintenanceCleanup() {
        ...
        try {
            ...
            if (vocabulary.length > 1000) {
                const sortedVocab = vocabulary.sort((a, b) => new Date(b.lastReviewed).getTime() -
            ...
            if (sentences.length > 1000) {
                const sortedSentences = sentences.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    validateUserSettings(settings) {
        ...
        if (typeof settings.nativeLanguage !== 'string')
            return false;
        if (typeof settings.learningLanguage !== 'string')
            return false;
        if (typeof settings.difficultyLevel !== 'number' ||
    ...
    validateArticle(article) {
        ...
        if (typeof article.title !== 'string')
            return false;
        if (typeof article.originalLanguage !== 'string')
            return false;
        if (!Array.isArray(article.parts))
            return false;
        if (!['processing', 'completed', 'failed'].includes(article.processingStatus))
            return false;
        return true;
    }
    /**
    ...
    validateVocabulary(vocab) {
        if (!vocab || typeof vocab !== 'object')
            return false;
        if (typeof vocab.id !== 'string' || !vocab.id)
            return false;
        if (typeof vocab.word !== 'string' || !vocab.word)
            return false;
        if (typeof vocab.translation !== 'string')
            return false;
        if (typeof vocab.context !== 'string')
            return false;
        if (!Array.isArray(vocab.exampleSentences))
            return false;
        if (typeof vocab.articleId !== 'string')
            return false;
        if (typeof vocab.partId !== 'string')
            return false;
        if (typeof vocab.reviewCount !== 'number')
            return false;
        if (typeof vocab.difficulty !== 'number' ||
            vocab.difficulty < 1 ||
            vocab.difficulty > 10)
            return false;
        return true;
    ...
    validateSentence(sentence) {
        ...
        if (typeof sentence.id !== 'string' || !sentence.id)
            return false;
        if (typeof sentence.content !== 'string' || !sentence.content)
            return false;
        if (typeof sentence.translation !== 'string')
            return false;
        if (typeof sentence.articleId !== 'string')
            return false;
        if (typeof sentence.partId !== 'string')
            return false;
        return true;
    }
}
 ...
 */
export function getStorageManager(cacheManager) {
 ...
 */
export function createStorageManager(cacheManager) {
