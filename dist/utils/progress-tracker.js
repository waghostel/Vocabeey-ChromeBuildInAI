...
export class ProgressTracker {
    ...
    callbacks = [];
    updateInterval = null;
    constructor(type, total, message = '') {
        this.progressState = {
            type,
            current: 0,
            total,
    ...
    calculatePercentage() {
        if (this.progressState.total === 0)
            return 0;
        return Math.round((this.progressState.current / this.progressState.total) * 100);
    }
    /**
     * Calculate estimated time remaining
     */
    calculateETA() {
        if (this.progressState.current === 0)
            return undefined;
...
export class ArticleProcessingTracker extends ProgressTracker {
     ...
     */
    updateStep(partIndex, step, progress = 0) {
     ...
     */
    streamWords(partIndex, currentWord, totalWords) {
...
export class StreamingProgressHandler {
    tracker;
    options;
    constructor(type, total, message, options = {}) {
     ...
     */
    async streamText(text, delayMs = 50) {
...
export class LoadingStateManager {
     ...
     */
    onLoadingChange(operationId, callback) {
     ...
     */
    offLoadingChange(operationId, callback) {
        const callbacks = this.callbacks.get(operationId);
        if (callbacks) {
            this.callbacks.set(operationId, callbacks.filter(cb => cb !== callback));
        }
    }
    /**
     * Clear loading state
     ...
     * Notify callbacks
     */
    notifyCallbacks(operationId, loading) {
        const callbacks = this.callbacks.get(operationId);
        if (callbacks) {
     ...
     */
    async withLoading(operationId, operation) {
...
export class ProgressUIBuilder {
     ...
     */
    createStreamingDisplay(text, isComplete = false) {
