...
let currentTTSRetryIndicator = null;
/**
 * Pronounce text with visual feedback on the clicked element
 */
export async function pronounceText(element, text, language) {
    ...
    try {
        const { speak, stopSpeaking, isTTSSupported } = await import('../utils/tts-service.js');
 ...
 */
export async function cancelTTS() {
    if (currentTTSAbortController) {
        currentTTSAbortController.abort();
        currentTTSAbortController = null;
    }
 ...
 */
function removeSpeakingIndicators() {
    ...
    if (speakingElements.length > 0) {
        console.log('TTS: Cleaning up', speakingElements.length, 'orphaned .speaking classes');
 ...
 */
function updateTTSRetryIndicator(text) {
    ...
    const messageElement = currentTTSRetryIndicator.querySelector('.tts-retry-message');
    if (messageElement) {
 ...
 */
function showTTSRetryIndicator(text) {
    if (currentTTSRetryIndicator) {
 ...
 */
function hideTTSRetryIndicator() {
    if (currentTTSRetryIndicator) {
        currentTTSRetryIndicator.remove();
        currentTTSRetryIndicator = null;
    }
}
//# sourceMappingURL=tts-handler.js.map